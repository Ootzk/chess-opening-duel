...
- **2.5점 이상** 획득하고 **상대보다 높은 점수**를 가진 플레이어가 시리즈 승자
- 5경기 내에 승자가 결정되지 않으면 (예: 2.5-2.5 동점) **서든데스** 진행
- 서든데스에서는 점수 차이가 발생할 때까지 계속 진행

### 밴픽 플로우

1. **Pick Phase** (30초): 10개 오프닝 프리셋 중 정확히 5개 선택
   - 타임아웃: 현재 선택 + 랜덤으로 5개 채워서 자동 확정
2. **Ban Phase** (30초): 상대의 픽 중 정확히 2개 밴
   - 타임아웃: 현재 선택 + 랜덤으로 2개 채워서 자동 확정
3. **Game 1**: 밴된 4개 + 중립(Standard Game) = 5개 중 랜덤 선택
